...
Grid.nxC = round(2/1*(128+64)*LWRatio) #round( RefinementFac*(Grid.ymax-Grid.ymin)/ CompactionLength)
Grid.nyC = 2/1*((128+64))#round( RefinementFac*(Grid.xmax-Grid.xmin)/ CompactionLength)
...
# Weak Layer
i+=1
Geometry["%05d_line" % i] = Input.Geom_Line(WeakPhase,slope,Hweak +  ThickWeak - slope*W,"y","<",Grid.xmin+W*.4,Grid.xmin+Lweak)
i+=1
Geometry["%05d_line" % i] = Input.Geom_Line(SedPhase,slope,Hweak - slope*W,"y","<",Grid.xmin+W*.4,Grid.xmin+Lweak)
...
Geometry["%05d_sine" % i] = Input.Geom_Sine(BasementPhase,Hbase - slope*W,0*0.25*Hbase,pi/16,Wseamount*0.20,"y","<",Grid.xmin+W*.4,Grid.xmax)
#i+=1
...
BCStokes.Sandbox_TopSeg00 = Hbase + ThickWeak
BCStokes.Sandbox_TopSeg01 = Hbase + ThickWeak
