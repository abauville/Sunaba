...
ResFacList = [.25,.5,2,4,8]
nx0 = Grid.nxC
ny0 = Grid.nyC
for ResFac in ResFacList:    
    Grid.nxC = int(nx0)*ResFac
    Grid.nyC = int(ny0)*ResFac
    dt_stressFac = 1e-3
    ##              Non Dim
    ## =====================================
...
#    Sy_back = C*cos(phi) + P*sin(phi)
    ...
    if ProductionMode:
        Output.folder = "/Users/abauville/Work/Paper_DynStress/Output/gridDependence/Production/dtstressFac_%.1e_ResFac_%.1e" % (Numerics.dt_stressFac, ResFac)
    else:
        Output.folder = "/Users/abauville/Work/Paper_DynStress/Output/gridDependence/Test/dtstressFac_%.1e_ResFac_%.1e" % (Numerics.dt_stressFac, ResFac)
