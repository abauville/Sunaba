...
Patch       = Input.Material("Sediments")


Setup.MatProps = {"0":StickyAir,"1":Sediment,"2":Decollement,"3":Patch}
...
# Static Fluid Pressure Factor
Patch.staticPfFac = LambdaPatch

# StrainWeakening
Patch.staticPfFacWeakFac = PfWeakFacPatch
Patch.frictionAngleWeakFac = frictionWeakFacPatch
Patch.cohesionWeakFac = cohesionWeakFacPatch#weakFac # 0.0 is none weak, 1.0 is fully weakened Cfinal = Cini*(1-CweakFac)

Patch.strainWeakStart = Sediment.strainWeakStart
Patch.strainWeakEnd = Sediment.strainWeakEnd
...
Patch.frictionAngle  = fricAnglePatch
...
Patch.cohesion = 1.0*Sediment.cohesion
...
Patch.vDisl = material.DislocationCreep     (eta0=RefVisc*ViscFac, n=1)
...
slope = 0.0*pi/180.0
#Hdec = Hsed*Hdec
#Geometry["%05d_line" % i] = Input.Geom_Line(PatchPhase,slope,Hdec - slope*(L-Lwedge)*Hdec,"y","<",Grid.xmin+PatchDistFromLeft,Grid.xmin+PatchDistFromLeft+PatchL)
Geometry["%05d_line" % i] = Input.Geom_Line(PatchPhase,slope,Hdec - slope*(L-Lwedge)*Hdec,"y","<",Grid.xmin+PatchDistFromLeft*Hsed,Grid.xmin+(PatchDistFromLeft+PatchL)*Hsed)
