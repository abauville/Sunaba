...
Sediment.cohesion =  15.0*MPa# * 20.0
...
Grid.nxC = round(Htotal*32*LWRatio) #round( RefinementFac*(Grid.ymax-Grid.ymin)/ CompactionLength)
Grid.nyC = round(Htotal*32)#round( RefinementFac*(Grid.xmax-Grid.xmin)/ CompactionLength)
...
if ProductionMode:
    baseFolder = "/Users/abauville/Output/Paper_Decollement/Static/Beta0/NewWeak_C%.2f_Weak%.f_Lambda%.f_G%.f_swIni%.1f_swEnd%.1f/" % (Sediment.cohesion/MPa,Sediment.cohesionWeakFac*100,Lambda*100, Sediment.G/1e8, Sediment.strainWeakStart, Sediment.strainWeakEnd)
else:
...
Sy_back = Sy_back # Half the max Sy is more representative
