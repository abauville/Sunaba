...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb  4 15:49:53 2019
...
chi_list = [20]
Lambda_list = arr([60])
tstep_list = np.arange(000,2001,50)
#tstep_list = np.concatenate([tstep_list,np.arange(400,550,5)])
#tstep_list = np.concatenate([tstep_list,np.arange(550,801,50)])
...
#stepMohr = arr([1000,1300,1500,1800])
stepMohr = arr([1600,2000])
...
#fig  = Figz_Utils.Figure(106,height=21.0,width=21.0,mode='production')
...
fig3  = Figz_Utils.Figure(108,height=29.7,width=21.0,mode='production')
AxesSMaps= Figz_Utils.makeAxes(fig3,4,1,bottomMarginPad=4.5)
...
if Compute:
    ...
    #yP = .65
    ymaxPlot = 6.0
    xminPlot = -1.0/aspectRatio*ymaxPlot
    tstep = 2000
#    xP = -3.5
#    yP = 2.2
    ...
    RefxP2D,RefyP2D = np.meshgrid(np.arange(xminPlot,0.1,0.25),np.arange(0,ymaxPlot,0.05))
    RefxP = RefxP2D.flatten()
    RefyP = RefyP2D.flatten()
    inWedge = np.zeros(RefxP.shape)==1
    ...
    xPIni = []#np.zeros(xP.shape)
    yPIni = []#np.zeros(xP.shape)
    for i in range(RefxP.shape[0]):
        I = np.argmin((PartX-RefxP[i])**2 + (PartY-RefyP[i])**2)
        Dist = np.sqrt((PartX[I]-RefxP[i])**2 + (PartY[I]-RefyP[i])**2)
        if Dist<0.1:
            inWedge[i] = True
            xPIni.append(PartXIni[I])
            yPIni.append(PartYIni[I])
            
    xPIni = np.array(xPIni)
    yPIni = np.array(yPIni)
#    inWedge = np.reshape(xP2D.shape)
    plt.plot(RefxP,RefyP,'or')
    ...
    #xPIni = np.linspace(-12.5,-12.,n)
    #xPIni = np.linspace(-14.0,-12.,n)
    #yPIni = arr([.15])
...
PovMax = np.zeros(RefxP.shape)
PovMin = np.zeros(RefxP.shape)
PMax = np.zeros(RefxP.shape)
PMin = np.zeros(RefxP.shape)
iPin = 0
Lambda = 0.6
I_Interesting = []

stress_scale = (1.0-Lambda)*rho*g*Char.length
#stress_scale = 1e6 


for iP in range(len(RefxP)):
    if inWedge[iP]==True:
        IMax = np.argmax(Pressure[:,iPin])
#        IMax = np.argmax(Pressure[:,iPin]/Pl[:,iPin])
#        IMin = np.argmin(Pressure[:,iPin]/Pl[:,iPin])
#        IMin = np.min(Pressure[IMax:,iPin]/Pl[IMax:,iPin])
        
        PovMax[iP] = np.max(Pressure[IMax:,iPin]/Pl[IMax:,iPin])
        
        PovMin[iP] = np.min(Pressure[IMax:,iPin]/Pl[IMax:,iPin])
        
#        PMax[iP] = Pressure[IMax,iPin]
#        PMin[iP] = Pressure[IMin,iPin]
        
#        IMax = np.argmax(Pressure[:,iPin])
        PMax[iP] = Pressure[IMax,iPin]
        PMin[iP] = np.min(Pressure[IMax:,iPin])
        
        if (1.0-Lambda)*(PMax[iP]-PMin[iP])/stress_scale>1.2 and (1.0-Lambda)*(PMax[iP])/stress_scale>4.0:
            I_Interesting.append(iPin)
        ...
        PovMax[iP] = np.nan
        PovMin[iP] = np.nan
PovMax = np.reshape(PovMax,RefxP2D.shape)
PovMin = np.reshape(PovMin,RefxP2D.shape)
plt.sca(AxesSMaps['21'])
vmin = .5
vmax = 1.5
plt.contourf(RefxP2D,RefyP2D,PovMax,np.linspace(vmin,vmax,20))
plt.set_cmap('jet')
plt.text(xmin+.25,ymax-.5,'max(P/Plitho)',fontfamily='Times New Roman',weight='bold',size=14)
plt.axis([xmin,xmax,ymin,ymax])
plt.colorbar()
#plt.colorbar(ticks=np.linspace(vmin,vmax,5))

plt.sca(AxesSMaps['31'])
vmin = .5
vmax = 1.5
plt.contourf(RefxP2D,RefyP2D,PovMin,np.linspace(vmin,vmax,20))
#plt.contourf(RefxP2D,RefyP2D,PovMin)
plt.set_cmap('jet')
plt.colorbar()
#plt.colorbar(ticks=np.linspace(vmin,vmax,5))
plt.text(xmin+.25,ymax-.5,'min(P/Plitho)',fontfamily='Times New Roman',weight='bold',size=14)
plt.axis([xmin,xmax,ymin,ymax])
#
plt.sca(AxesSMaps['41'])
vmax = 8.0/9.0
plt.contourf(RefxP2D,RefyP2D,PovMax-PovMin,np.linspace(.0,vmax,20))
plt.set_cmap('jet')
plt.colorbar(ticks=np.linspace(.0,vmax,3))
plt.text(xmin+.25,ymax-.5,'max-min',fontfamily='Times New Roman',weight='bold',size=14)
plt.axis([xmin,xmax,ymin,ymax])
                
plt.sca(AxesSMaps['11'])        


#plt.plot((1.0-Lambda)*(PMax-PMin)/1e6,(1.0-Lambda)*PMax/1e6,'o')
plt.plot((1.0-Lambda)*(PMax-PMin)/stress_scale,(1.0-Lambda)*PMax/stress_scale,'o')

#plt.plot(PovMin,PovMax,'o',markerSize=.5)
#plt.plot(PovMax-PovMin,PovMax,'o',markerSize=.5)
#plt.plot([8.0/9.0,8.0/9.0],[1.0,2.25],':k',linewidth=.5)

Is = np.argwhere(PovMax-PovMin>8.0/9.0)

#plt.plot(PovMin,PovMax,'o',markerSize=.5)
#plt.plot(PMin,PMax,'o',markerSize=.5)

plt.ylabel('PMax')
plt.xlabel('PMax-PMin')




## Plot stress history of a given particle
...
#plt.fill(np.concatenate([tstep_list,np.flipud(tstep_list)]),np.concatenate([ ((Pressure)*(1.0-Lambda)+Ty)/1e6,np.flipud((Pressure)*(1.0-Lambda)-Ty)/1e6 ]),'k',alpha=.05)
plt.plot(tstep_list,(S1[:,iP])/stress_scale,'-or',linewidth=.5,markersize=.75,)
plt.plot(tstep_list,(Pressure[:,iP])*(1.0-Lambda)/stress_scale,'-ok',linewidth=.5,markersize=.75)
plt.plot(tstep_list,(S3[:,iP])/stress_scale,'-og',linewidth=.5,markersize=.75)
plt.plot(tstep_list,(Pl[:,iP])*(1.0-Lambda)/stress_scale,'--b',linewidth=.5,markersize=.75)
