...
thisFile_chi_list = [1, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60, 65]
...
# =====
AxesDrawing     = Figz_Utils.makeAxes(fig,1,int(nSim/2),aspectRatio=aspectRatioDrawing,
                                      leftMarginPad=1.0,rightMarginPad=0.0,
                                      topMarginPad = yShift,bottomMarginPad = 0.0,
                                      xPad=xPad,yPad=yPad,
                                      setAspectRatioBasedOn='x')
yShift0 = AxesDrawing['info']['plotsHeight']+AxesDrawing['info']['yPad']
yShift += yShift0
AxesAlpha       = Figz_Utils.makeAxes(fig,1,int(nSim/2),aspectRatio=aspectRatio,
                                      leftMarginPad=1.0,rightMarginPad=0.0,
...
yShift0 = AxesAlpha['info']['plotsHeight']+AxesAlpha['info']['yPad']
yShift += yShift0
AxesxFault      = Figz_Utils.makeAxes(fig,1,int(nSim/2),aspectRatio=aspectRatio,
                                      leftMarginPad=1.0,rightMarginPad=0.0,
                                      topMarginPad=yShift,bottomMarginPad = bottomMarginPad,
                                      xPad=xPad,
                                      setAspectRatioBasedOn='x')
...
yShift += yShift0
AxesDrawing2     = Figz_Utils.makeAxes(fig,1,int(nSim/2),aspectRatio=aspectRatioDrawing,
                                      leftMarginPad=1.0,rightMarginPad=0.0,
                                      topMarginPad = yShift,bottomMarginPad = 0.0,
                                      xPad=xPad,yPad=yPad,
                                      setAspectRatioBasedOn='x')
...
yShift0 = AxesDrawing['info']['plotsHeight']+AxesDrawing['info']['yPad']
yShift += yShift0
AxesAlpha2       = Figz_Utils.makeAxes(fig,1,int(nSim/2),aspectRatio=aspectRatio,
                                      leftMarginPad=1.0,rightMarginPad=0.0,
                                      topMarginPad = yShift,bottomMarginPad = bottomMarginPad,
                                      xPad=xPad,yPad=yPad,
                                      setAspectRatioBasedOn='x')
...
yShift0 = AxesAlpha['info']['plotsHeight']+AxesAlpha['info']['yPad']
yShift += yShift0
AxesxFault2      = Figz_Utils.makeAxes(fig,1,int(nSim/2),aspectRatio=aspectRatio,
                                      leftMarginPad=1.0,rightMarginPad=0.0,
                                      topMarginPad=yShift,bottomMarginPad = bottomMarginPad,
                                      xPad=xPad,yPad=yPad,
                                      setAspectRatioBasedOn='x')
...
for i in range(int(nSim/2)):
    AxesAlpha['1%i' % (nSim/2+i+1)] = AxesAlpha2['1%i' % (i+1)]
    AxesxFault['1%i' % (nSim/2+i+1)] = AxesxFault2['1%i' % (i+1)]
    AxesDrawing['1%i' % (nSim/2+i+1)] = AxesDrawing2['1%i' % (i+1)]
#
...
for iSim in range(nSim):
    ...
    ax = plt.sca(AxesDrawing["1%i" % (iSim+1)])
    ...
    ymax = 4.
    plt.axis([-1.0/aspectRatioDrawing*ymax,0.0,0.0-0.25,ymax-0.25])
#    plt.axis("off")
...
beta = 0.0
Lambda = 0.6
loadedData = np.load("/Users/abauville/Output/Paper_Decollement/Figz/Data/slopes_Beta%02d_Lambda%02d_WeakDense.npz" % (beta*10.0*180.0/np.pi,Lambda*100.0))
...
#        plt.sca(AxesxFault['%i1' % (iSim+1)])
        ...
        ax.spines['right'].set_visible(False)
        ax.spines['top'].set_visible(False)
        plt.plot(timeLists[iSim][I]/kyr,Setup.Grid.nxC-xFronts[iSim][I],'-',color=[.5,.5,.5],linewidth=1.0,markersize=.5)
        plt.plot(timeLists[iSim][I]/kyr,Setup.Grid.nxC-xMids[iSim][I]  ,'-',color=[.6,.1,.0],linewidth=1.0,markersize=.5)
        plt.plot(timeLists[iSim][I]/kyr,Setup.Grid.nxC-xBases[iSim][I] ,'-',color=[.0,.6,.1],linewidth=1.0,markersize=.5)
...
# Add Dalpha
# Change color in B from red to blue
# maybe change leave the arrow the C1 C3 cases in black and put the other ones in gray
