...
#        plt.streamplot(x,y,Vx.T,Vy.T,color="w", density=2.5,arrowsize=1)        
        ...
        phase = Output.getData(dataFolder + 'phase.bin',False)
        rx = 20
        ry = 20
        mask = phase==1
        mask_sub = mask[::rx,::ry]
        Sxx = Output.getData(dataFolder + 'sigma_xx.bin',True,mask=mask).data[::rx,::ry]
        Sxy = Output.getData(dataFolder + 'sigma_xy.bin',True,mask=mask).data[::rx,::ry]
        if iSim == 0:
...
#        psi = np.ma.masked_array(psi, mask_sub)
        ...
        length = 1.00 * phase[::rx,::ry].flatten()
        ...
        plt.plot( xCenter + xLine , yCenter + yLine ,'-k' )
