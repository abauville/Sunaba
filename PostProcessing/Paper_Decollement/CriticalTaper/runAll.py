    #!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Created on Mon Sep  3 16:09:39 2018

@author: abauville
"""
import matplotlib.pyplot as plt
dpi = 300
OutputFolder = "/Users/abauville/Output/Paper_Decollement/Figz/CritTaper/"

#print("Fig01")
#import CritTaper_Fig01
#plt.savefig(OutputFolder + "02_CritTaper_Fig01",dpi=dpi)
#
#print("Fig02")
#import CritTaper_Fig02
#plt.savefig(OutputFolder + "02_CritTaper_Fig02",dpi=dpi)
#
#print("Fig03")
#import CritTaper_Fig03
#plt.savefig(OutputFolder + "02_CritTaper_Fig03",dpi=dpi)
#
#print("Fig04")
#import CritTaper_Fig04
#plt.savefig(OutputFolder + "02_CritTaper_Fig04",dpi=dpi)
#
#print("Fig05")
#import CritTaper_Fig05
#plt.savefig(OutputFolder + "02_CritTaper_Fig05",dpi=dpi)
#
#print("Fig06")
#import CritTaper_Fig06
#plt.savefig(OutputFolder + "02_CritTaper_Fig06",dpi=dpi)
#
print("Fig07")
import CritTaper_Fig07
plt.savefig(OutputFolder + "02_CritTaper_Fig07",dpi=dpi)
#
#print("Fig07Mod")
#import CritTaper_Fig07Mod
#plt.savefig(OutputFolder + "02_CritTaper_Fig07Mod",dpi=dpi)