...
fig = Figz_Utils.Figure(4,height=9.25)
...
LambdaShortList = np.array([0.01, 0.4, 0.9])
iTapers = []
...
for iTaper in iTapers:
    ...
    for iSub in range(len(chiShortList)):
        ...
        # Plot fully weakened
        tpr = Taper_WF[iTaper*nChi+I]
        plt.fill(tpr.beta_all*deg, (tpr.alpha_all)*deg,facecolor="None",edgecolor="b",linestyle=linestyle[iSub])
...
#    plt.xlabel("$\\beta$ [°]")
    ...
    ax.text(x1-(x1-x0)*0.4,y1-(y1-y0)*0.1,'$\\lambda=%i$ %%' % (LambdaShortList[i]*100))
    i+=1
