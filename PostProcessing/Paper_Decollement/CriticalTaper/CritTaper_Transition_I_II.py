...
#            alphas_diff_all2[iL,iW,iB] = alphas_diff_all[iL,iW,IB]
        # end iW
        I = np.argmax(alphas_diff)
        taperAngle_transition_I_II [iL, iB] = taper_angles_all[iL,I,IB]
        I = np.argmin(np.abs(alphas_diff))
        taperAngle_transition_II_III[iL, iB] = taper_angles_all[iL,I,IB]
    # end iB
#    plt.plot(betas*180.0/np.pi, chi_transition[iL, :],'-o')    
