...
#    AxesDum['12'].axis('off')
Axes   = Figz_Utils.makeAxes(fig,nrows,ncols,aspectRatio=1.0,leftMarginPad=1.5,rightMarginPad=1.0,topMarginPad = 0.0,xPad = 0.1,yPad = 1.0)
...
for iSim in range(iSim0,nSim):
    ...
    if plot==0:
        ...
        if iCol == 0:
            ...
            y1 = np.max([alpha_WB_up,alpha_Ref])*2.0*deg
#            plt.ylim([y0,y1])
    ...
    #    plt.plot(timeLists[iSim]/kyr,slopes[iSim]*deg,'ok',markersize=1.0)
        plt.plot(timeLists[iSim]/kyr,slopes[iSim]*deg,'-ok',linewidth=.5,markersize=.5)
...
##        y1 = 35.0
        ...
        I = np.arange(5,len(timeLists[iSim])-2)
        plt.plot(timeLists[iSim][I]/kyr,Setup.Grid.nxC-xFronts[iSim][I],'-ok',linewidth=.5,markersize=.5)
        plt.plot(timeLists[iSim][I]/kyr,Setup.Grid.nxC-xMids[iSim][I],'-or',linewidth=.5,markersize=.5)
        plt.plot(timeLists[iSim][I]/kyr,Setup.Grid.nxC-xBases[iSim][I],'-ob',linewidth=.5,markersize=.5)
