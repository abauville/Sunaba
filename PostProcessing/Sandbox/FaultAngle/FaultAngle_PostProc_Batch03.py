...
simFolder  = "HSed1_SA0_C10000_FA8/"
inFolder  = "Input/"
outFolder = "Out_00000/"
...
Syst_HSed               = [0.001*km, 0.01*km, 0.1*km, 1.0*km, 10.0*km]
...
#Syst_HSed               = [0.001*km]
#Syst_surfaceAngleAngle  = [0] # in degrees
#Syst_frictionAngle      = [8]    # in degrees
#Syst_cohesion           = [0.01*MPa]
...
for thisHSed in Syst_HSed:
    ...
    for thisCohesion in Syst_cohesion:
        plt.clf()
        plt.axis('equal')
        print("\nHSed: " + str(thisHSed/1e3) + " km" + ", C: " + str(thisCohesion/1e6) + " MPa")
        ...
        for thisSurfaceAngle in Syst_surfaceAngleAngle:
            ...
            for thisFrictionAngle in Syst_frictionAngle:
                ...
                try:
                    ...
                    thisData = Output.getData(rootFolder + simFolder + outFolder + 'strainRate.bin')
                    #data = np.transpose(thisData.data)
                    data = np.ma.masked_array(thisData.data, yv > (Hsed_nondim+tan(+thisSurfaceAngle*degree)*xv)) # Hide the air
                             
                    # Plot
                    # =====================        
                    plt.pcolor(xv+xShift,yv+yShift,np.log10(data), vmin=0, vmax=4.0)
                    ...
                    #simFolder = "/Users/abauville/StokesFD_Output/FaultAngle_Systematics_Batch02/" + "HSed" + str(round(thisHSed)) + "_SA" + str(round(thisSurfaceAngle)) + "_C" + str(round(thisCohesion)) + "_FA" + str(round(thisFrictionAngle))
                    print(rootFolder + simFolder + outFolder + 'strainRate.bin' + " could not be loaded")
        ...
        plt.title( "Hsed = " + str(thisHSed/1e3) + " km" + ", Cohesion = " + str(thisCohesion/1e6) + " MPa")
        
        plt.axis([(W-WShift), (ix)*WShift+xShiftIni+(WShift-W), (H-HShift), (iy)*HShift+yShiftIni+(HShift-H)])
        #plt.show()
        
        #plt.savefig("Batch0#/" + "HSed" + str(round(thisHSed)) + "_C" + str(round(thisCohesion)) + ".png", dpi=500)
        plt.savefig("/Users/abauville/GoogleDrive/Output/" + "HSed" + str(round(thisHSed)) + "_C" + str(round(thisCohesion)) + ".png", dpi=500)
