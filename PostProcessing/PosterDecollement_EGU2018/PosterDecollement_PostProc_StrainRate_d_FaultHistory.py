...
WcBar = 0.5
HcBar = 0.025
axDict["axColorBar"] = plt.axes([0.5-WcBar/2.0, 0.09, WcBar, HcBar],xticks=np.arange(cAx_srMin,cAx_srMax+0.0000001,0.5))
...
if Compute:   
    for iSim in range(0,nSim):
        if iSim==0:
            i0=2920
            nSteps = 3701
            jump= int(np.floor(((nSteps-i0)-1)/(nSubPlots-1)))
        elif iSim==1:
            i0=2920
            nSteps = 3251
            jump= int(np.floor(((nSteps-i0)-1)/(nSubPlots-1)))
...
#            imgplot2 = plt.imshow(imgStrainRate,extent=[xmin,xmax,ymin,ymax],cmap=cmap)
            ...
            # Scale
            if (iSub == 1):
                plt.fill(xminP+np.array([0.0,1000.0+200.0,1000.0+200.0,0.0]),yminP+np.array([0.0,0.0,75.0+500.0,75.0+500.0]),"w")
            ...
            pushV = 10.0*cm/yr
            Hsed = 1 * km
            short = timeSim*pushV/Hsed
#            if iSim==0:
            plt.text(xminP+100.0+100.0,yminP+75.0+100.0+1000.0,"sh. %.1f*H" % (short))
...
plt.savefig("/Users/abauville/Dropbox/00_ConferencesAndSeminars/EGU2018/Figz/DecollementHistory_b",dpi=1000)
